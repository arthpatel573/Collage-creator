...
def bound_blur(image, b, *args):
    ...
    for i in range(h):
        for j in range(w):
            output[i, j, 0:d] = (
                image[i, j, 0:d] * min(min(i, h - i) / b, 1) * min(min(j, w - j) / b, 1)
            )
    return output
...
def bound_recover(image, b, *args):
    ...
    for i in range(h):
        for j in range(w):
            output[i, j, 0:d] = (
                image[i, j, 0:d]
                * max(b / min(i + b_half, h - i + b_half), 1)
                * max(b / min(j + b_half, w - j + b_half), 1)
            )
    return output
...
def image_merge(images, b, vertical, horizontal, *args):
    ...
    output[0 : ver + b_half, 0 : hor + b_half, 0:d] += blur_1[
        b_half : ver + b, b_half : hor + b, 0:d
    ]
    output[ver - b_half : 480, 0 : hor + b_half, 0:d] += blur_2[
        0 : 480 - ver + b_half, b_half : hor + b, 0:d
    ]
    output[0 : ver + b_half, hor - b_half : 640, 0:d] += blur_3[
        b_half : ver + b, 0 : 640 - hor + b_half, 0:d
    ]
    output[ver - b_half : 480, hor - b_half : 640, 0:d] += blur_4[
        0 : 480 - ver + b_half, 0 : 640 - hor + b_half, 0:d
    ]

    return bound_recover(output, b)

@collage_api.route("", methods=["POST"])
def collage():
    """
    ...
    if auth == current_app.config["API_KEY"]:
        ...
        image_full_list = []
        for i in image_list:
            image_full_list.append(
                os.path.join("data", video_id + "_" + str(i) + ".jpg")
            )
        imageCollection = io.ImageCollection(image_full_list)
        collage = (
            image_merge(np.array(list(x for x in imageCollection)), 60, 0.5, 0.5) * 255
        ).astype("uint8")
